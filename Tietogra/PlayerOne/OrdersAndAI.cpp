			...
			if (unit.unitType != 'W' && unit.unitType != 'B') {
				...
				for (const auto& enemyUnit : activeUnits) {
					...
					if (enemyUnit.unitAffiliation == 'E') {
						...
						int distance = std::abs(startX - enemyX) + std::abs(startY - enemyY);
				...
				if (targetX < startX) {
					++adjacentX;  // Move to the left
				}
				else if (targetX > startX) {
					--adjacentX;  // Move to the right
				}
				...
				if (baseAttacked) {
					...
					executedCommands.push_back(attackCommand);  // Add the executed command for Move
				}

				if (nearestEnemyID != -1 && standardAttack) {
					attackCommand.unitID = unit.ID;
